...
static const unsigned char intro_xml[] =
	"<node>"
	"  <interface name='com.thundersoft.approotn1.Config.Man'>"
  	"      <property type='s' name='gender' access='readwrite'/>"
	"  </interface>"
	"  <interface name='com.thundersoft.approotn1.Config.secweather'>"
	...
	"  </interface>"
	"  <interface name='com.thundersoft.approotn1.Config.Yellow'>"
...
static void _get_uci_section_val(const char *section_name, const char *opt_name, char *opt_val)
{
        ...
        sprintf(cmd, "/sbin/uci get approotn.%s.%s", section_name, opt_name);
        fp = popen(cmd, "r");
...
static int
approotn_get_func	(const char	 *path,
		 const char	 *prop,
		 GVariant	**value,
		 void		 *userdata)
{
...
static int
approotn_set_func	(const char	 *path,
		 const char	 *prop,
		 GVariant	 *value,
		 void		 *userdata)
{
	...
	{
		sprintf(cmd, "/sbin/uci set approotn.testsec.%s=\'%s\'", prop, val);
	}
	else if (g_strcmp0 (prop, "gender") == 0)
	{
		sprintf(cmd, "/sbin/uci set approotn.Man.%s=\'%s\'", prop, val);
	}
	else if ((g_strcmp0 (prop, "day") == 0) ||
	    	 (g_strcmp0 (prop, "date") == 0) || 
	    	 (g_strcmp0 (prop, "weath") == 0))
	{
		sprintf(cmd, "/sbin/uci set approotn.secweather.%s=\'%s\'", prop, val);
	}
	else if ((g_strcmp0 (prop, "shape") == 0) ||
	    	 (g_strcmp0 (prop, "colour") == 0))
	{
		sprintf(cmd, "/sbin/uci set approotn.Yellow.%s=\'%s\'", prop, val);
	}

	g_print ("set_func: %s\n", cmd);
        system(cmd);
	sleep (1);
	g_print ("set_func: /sbin/uci commit approotn\n");
        system("/sbin/uci commit approotn");

	return 0;
}


static CtSgwGDBusInterfaceVTable_t interface_vtable = { handle_method_call, approotn_get_func, approotn_set_func, NULL, NULL };


static int approotn_stop(void)
{
...
static int approotn_reload(void)
{
...
static void test_register_methods(CtSgwDBusNodeInfo_t *intro)
{
	...
	// path: /com/thundersoft/approotn1/Config/Man/Man
	// intf: com.thundersoft.approotn1.Config.Man
	registration_id = CtSgwDBusRegisterObject("/com/thundersoft/approotn1/Config/Man/Man",
					  intro->interfaces[0],
					  &interface_vtable,
					  NULL);
	g_assert(registration_id > 0);
	
	// path: /com/thundersoft/approotn1/Config/secweather/secweather
	// intf: com.thundersoft.approotn1.Config.secweather
	registration_id = CtSgwDBusRegisterObject("/com/thundersoft/approotn1/Config/secweather/secweather",
					  intro->interfaces[1],
					  &interface_vtable,
					  NULL);
	g_assert(registration_id > 0);
	
	// path: /com/thundersoft/approotn/Config/Yellow/Yellow
	// intf: com.thundersoft.approotn.Config.Yellow
	registration_id = CtSgwDBusRegisterObject("/com/thundersoft/approotn1/Config/Yellow/Yellow",
					  intro->interfaces[2],
					  &interface_vtable,
					  NULL);
	g_assert(registration_id > 0);
...
int main(int argc, char *argv[])
{
	...
	CtSgwAppMgtCallbacks_t app_cbs = { 
		.stop = approotn_stop,
		.reload = approotn_reload,
		.restore_factory = NULL,
		.post_msg = NULL
	};

	CtSgwLogOpen(LOG_USER, "approotn");

	CtSgwLog(LOG_NOTICE, "start\n");
	
	CtSgwDBusStartService("com.thundersoft.approotn1");
	...
	CtSgwLog(LOG_NOTICE, "approotn Running\n");
	//g_print("ctsgw Running\n");
	while (running) {
		sleep(1);
	}
	//g_print("TESTAPP Exiting\n");
	CtSgwLog(LOG_NOTICE, "approotn Exiting\n");
