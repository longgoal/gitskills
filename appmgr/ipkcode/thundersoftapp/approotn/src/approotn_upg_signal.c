		 ...
		 void		 *userdata)
{
	...
	g_dbus_connection_emit_signal (sgw_dbus_service_context->connection, NULL, "/com/thundersoft/tsappj1/Config/secweather/secweather",
					"com.thundersoft.tsappj1.Config.secweather", "alibaba-signal", g_variant_new ("(sv)", prop, value), NULL); 
...
int main(int argc, char *argv[])
{
	...
	CtSgwLog(LOG_USER, "tsappj register Managerment Function\n");
	mm_data = CtSgwAppRegisterMgtFuncs(&app_cbs);
	CtSgwLog(LOG_USER, "tsappj register Managerment Function End\n");

	CtSgwLog(LOG_USER, "tsappj Running\n");
	g_print("TSAPPJ Running\n");
	while (running) {
		sleep(1);
	}
	g_print("TSAPPJ Exiting\n");
	CtSgwLog(LOG_USER, "tsappj Exiting\n");
