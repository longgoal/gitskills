		 ...
		 void		 *userdata)
{
	...
	g_dbus_connection_emit_signal (sgw_dbus_service_context->connection, NULL, "/com/thundersoft/tsappcinterface1/Config/secweather/secweather",
					"com.thundersoft.tsappcinterface1.Config.secweather", "alibaba-signal", g_variant_new ("(sv)", prop, value), NULL); 
...
int main(int argc, char *argv[])
{
	...
	CtSgwLog(LOG_USER, "tsappcinterface register Managerment Function\n");
	mm_data = CtSgwAppRegisterMgtFuncs(&app_cbs);
	CtSgwLog(LOG_USER, "tsappcinterface register Managerment Function End\n");

	CtSgwLog(LOG_USER, "tsappcinterface Running\n");
	g_print("TSAPPJ Running\n");
	while (running) {
		sleep(1);
	}
	g_print("TSAPPJ Exiting\n");
	CtSgwLog(LOG_USER, "tsappcinterface Exiting\n");
