...
static const unsigned char intro_xml[] =
	"<node>"
	"  <interface name='com.thundersoft.tsappd1.Config.Man'>"
  	"      <property type='s' name='gender' access='readwrite'/>"
	"  </interface>"
	"  <interface name='com.thundersoft.tsappd1.Config.secweather'>"
	...
	"  </interface>"
	"  <interface name='com.thundersoft.tsappd1.Config.Yellow'>"
...
static void _get_uci_section_val(const char *section_name, const char *opt_name, char *opt_val)
{
        ...
        sprintf(cmd, "/sbin/uci get tsappd.%s.%s", section_name, opt_name);
        fp = popen(cmd, "r");
...
static int
tsappd_get_func	(const char	 *path,
		 const char	 *prop,
		 GVariant	**value,
		 void		 *userdata)
{
...
static int
tsappd_set_func	(const char	 *path,
		 const char	 *prop,
		 GVariant	 *value,
		 void		 *userdata)
{
	...
	{
		sprintf(cmd, "/sbin/uci set tsappd.testsec.%s=\'%s\'", prop, val);
	}
	else if (g_strcmp0 (prop, "gender") == 0)
	{
		sprintf(cmd, "/sbin/uci set tsappd.Man.%s=\'%s\'", prop, val);
	}
	else if ((g_strcmp0 (prop, "day") == 0) ||
	    	 (g_strcmp0 (prop, "date") == 0) || 
	    	 (g_strcmp0 (prop, "weath") == 0))
	{
		sprintf(cmd, "/sbin/uci set tsappd.secweather.%s=\'%s\'", prop, val);
	}
	else if ((g_strcmp0 (prop, "shape") == 0) ||
	    	 (g_strcmp0 (prop, "colour") == 0))
	{
		sprintf(cmd, "/sbin/uci set tsappd.Yellow.%s=\'%s\'", prop, val);
	}

        system(cmd);
        system("/sbin/uci commit tsappd");

#else
	//CtSgwDBusEmitSignal(NULL, "/com/upointech/alibaba", "com.upointech.alibaba", "alibaba-signal", g_variant_new("sv", prop, value));
	g_dbus_connection_emit_signal (sgw_dbus_service_context->connection, NULL, "/com/thundersoft/tsappd1/Config/secweather/secweather",
					"com.thundersoft.tsappd1.Config.secweather", "alibaba-signal", g_variant_new ("(sv)", prop, value), NULL); 
...
static CtSgwGDBusInterfaceVTable_t interface_vtable = { handle_method_call, tsappd_get_func, tsappd_set_func, NULL, NULL };


static int tsappd_stop(void)
{
	running = 0;
...
static void test_register_methods(CtSgwDBusNodeInfo_t *intro)
{
	...
	// path: /com/thundersoft/tsappd1/Config/Man/Man
	// intf: com.thundersoft.tsappd1.Config.Man
	registration_id = CtSgwDBusRegisterObject("/com/thundersoft/tsappd1/Config/Man/Man",
					  intro->interfaces[0],
					  &interface_vtable,
					  NULL);
	...
	// path: /com/thundersoft/tsappd1/Config/secweather/secweather
	// intf: com.thundersoft.tsappd1.Config.secweather
	registration_id = CtSgwDBusRegisterObject("/com/thundersoft/tsappd1/Config/secweather/secweather",
					  intro->interfaces[1],
					  &interface_vtable,
					  NULL);
	...
	// path: /com/thundersoft/tsappd/Config/Yellow/Yellow
	// intf: com.thundersoft.tsappd.Config.Yellow
	registration_id = CtSgwDBusRegisterObject("/com/thundersoft/tsappd1/Config/Yellow/Yellow",
					  intro->interfaces[2],
					  &interface_vtable,
					  NULL);
...
int main(int argc, char *argv[])
{
	...
	CtSgwAppMgtCallbacks_t app_cbs = { 
		.stop = tsappd_stop,
		.reload = tsappd_reload,
		.restore_factory = NULL,
		.post_msg = NULL
	};

	CtSgwLogOpen(LOG_USER, "tsappd");

	CtSgwLog(LOG_NOTICE, "start\n");
	
	CtSgwDBusStartService("com.thundersoft.tsappd1");

	intro_data = CtSgwDBusNodeParseXml(intro_xml);

	test_register_methods(intro_data);

	CtSgwLog(LOG_USER, "tsappd register Managerment Function\n");
	mm_data = CtSgwAppRegisterMgtFuncs(&app_cbs);
	CtSgwLog(LOG_USER, "tsappd register Managerment Function End\n");

	CtSgwLog(LOG_USER, "tsappd Running\n");
	g_print("TSAPPJ Running\n");
	while (running) {
		sleep(1);
	}
	g_print("TSAPPJ Exiting\n");
	CtSgwLog(LOG_USER, "tsappd Exiting\n");
