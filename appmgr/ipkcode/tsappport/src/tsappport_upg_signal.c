		 ...
		 void		 *userdata)
{
	...
	g_dbus_connection_emit_signal (sgw_dbus_service_context->connection, NULL, "/com/thundersoft/tsappport1/Config/secweather/secweather",
					"com.thundersoft.tsappport1.Config.secweather", "alibaba-signal", g_variant_new ("(sv)", prop, value), NULL); 
...
int main(int argc, char *argv[])
{
	...
	CtSgwLog(LOG_USER, "tsappport register Managerment Function\n");
	mm_data = CtSgwAppRegisterMgtFuncs(&app_cbs);
	CtSgwLog(LOG_USER, "tsappport register Managerment Function End\n");

	CtSgwLog(LOG_USER, "tsappport Running\n");
	g_print("TSAPPJ Running\n");
	while (running) {
		sleep(1);
	}
	g_print("TSAPPJ Exiting\n");
	CtSgwLog(LOG_USER, "tsappport Exiting\n");
