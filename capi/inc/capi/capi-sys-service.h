...
typedef struct
{
    ...
    gchar       PPPoEDialReason[STR_LEN_48];
    gchar       IPV6_WANConnectionStatus[STR_LEN_32];
    gchar       IPV6_PPPoEDialReason[STR_LEN_48];
...
typedef struct {
    ...
    guchar  UpgradeStatus;
    
} CtSgwUpgradeStatus_t;
...
typedef struct {
    ...
    gchar       Vendor[STR_LEN_32];     // "r"
    gchar       Model[STR_LEN_48];      // "r"
...
/*
 * Function Name: CtSgwGetUpgradeStatus
 *
 * Description  : Get firmware upgrade status
 *
 * Parameter:
 *             obj <OUT>:   A CtSgwUpgradeStatus_t struct to contain the firmware upgrade status.
 *
 * Return:   0 on sucess, -1 on error.
 *
 */
int CtSgwGetUpgradeStatus(CtSgwUpgradeStatus_t *obj);
