...
const function_entry_t FUCTION_MAP[NUMBER] = {
	...
	{ "CtSgw_AddDNSTunnel",CtSgw_AddDNSTunnel_NUM},
	{ "CtSgw_DelDNSTunnel", CtSgw_DelDNSTunnel_NUM},
...
Process_test_case(testcase->child,index);
		}
               	    }
                else{
...
int Process_test_exception(gchar* extral){
	...
	{       
		...
		//g_print("Test Running Process_test_module6 %p\n",testcase->child);
                ...
                g_print("=========Process_test_case===before");
		Process_test_case(testcase->child,index);
                g_print("=========Process_test_case===end");
		}
...
int Process_test_case(cJSON* testcase,int indexnumber){
...
g_print("###################################################\n");
	...
	case CtSgwGetWANConnectionObjs_NUM:{
			...
			char indexcontent="";
			indexcontent=strcpy((char)index, "CtSgwGetWANConnectionObjs");
			 ...
			 start = (double)t_start.tv_sec * 1000 + (double)t_start.tv_usec / 1000;
			   ...
			   if(FALSE == Check_return_result(return_value,atoi(return_target)))
			   {	
				 fill_capi_result(indexcontent,"failed",return_value,t_diff);
				g_free(out_param1);
				break;
			   }
			...
			t_diff = end - start;
			   ...
			   {
				fill_capi_result(indexcontent,"pass",return_value,t_diff);
				g_free(out_param1);
				break;
			   }
			   int index=0;
	...
	case CtSgwGetWLANDeviceObjs_NUM:{
				...
				char indexcontent="";
				indexcontent=strcpy((char)index, "CtSgwGetWLANDeviceObjs");
	...
	case CtSgwGetWiFiExTimer_NUM:{
			...
			t_diff = end - start; 
				...
				strcpy(target_param1.WeekDay,cJSON_GetObjectItem(CtSgwWiFiTimer1, "WeekDay")->valuestring); 
	...
	case CtSgwGetSleepTimer_NUM:{
			...
			t_diff = end - start;
				...
				strcpy(target_param1.Day,cJSON_GetObjectItem(CtSgwSleepTimer, "Day")->valuestring); 
	...
	case CtSgwAddVPNConnection_NUM:{
				 ...
				 strcpy(obj.terminal_mac,cJSON_GetObjectItem(CtSgwVPNConn, "terminal_mac")->valuestring); 
...
obj.ips = (CtSgwStrArray_t*)malloc(2 * sizeof(CtSgwStrArray_t));
	...
	case CtSgwGetVPNConnectionStatus_NUM:{
			 ...
			 start = (double)t_start.tv_sec * 1000 + (double)t_start.tv_usec / 1000;
				 ...
				 return_value = CtSgwGetVPNConnectionStatus(&out_param1);
	...
	case CtSgw_AddDNSTunnel_NUM:{
			...
			t_diff = end - start;
				 ...
				 if(FALSE == Check_return_result(return_value,atoi(return_target))) 
					 fill_capi_result("CtSgw_AddDNSTunnel","failed",return_value,t_diff); 
			         else
					 fill_capi_result("CtSgw_AddDNSTunnel","pass",return_value,t_diff); 
			 }
			 break;
	case CtSgw_DelDNSTunnel_NUM:{
			...
			t_diff = end - start;
				 ...
				 if(FALSE == Check_return_result(return_value,atoi(return_target))) 
					 fill_capi_result("CtSgw_DelDNSTunnel","failed",return_value,t_diff); 
			         else
					 fill_capi_result("CtSgw_DelDNSTunnel","pass",return_value,t_diff);
			 }
			 break;
	...
	case CtSgw_GetFTPAccountObjs_NUM:{
				...
				char indexcontent="";
				indexcontent=strcpy((char)index, "CtSgw_GetFTPAccountObjs");
	...
	case CtSgw_GetSambaAccountObjs_NUM:{
				...
				char indexcontent="";
				indexcontent=strcpy((char)index, "CtSgw_GetSambaAccountObjs");
